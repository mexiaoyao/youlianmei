 ...
 *1：两者均可用于导出常量，函数，文件，模块等
 *2：都可以使用import导入
 *
 *不同点
 *1：export可有多个，export default只能是一个
 *2：通过export导出，在导入是要加{}，export default则不需要
 *3：使用export default为模块指定默认输出，导入时只需要知道文件名即可，但是使用export使用export必须知道导出的变量或者函数等，导入时变量名必须一致
 *
 * 举例
 * //demo1.js
 * export default const a = "111111111"
 * 导入方式
 * import b from "demo1.js" 可以为"demo1"，此处的b可以是任意的变量
 * //demo2.js
 * export const a = "111111111"
 * 导入方式
 * import {a} from "demo2.js" 这里的a必须和导出的变量名一致
 *
 * 注意：
 * 一个js文件是可以有多个 export
